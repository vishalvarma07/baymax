...
router.post('/', credentialCheck, (req, res) => {
    ...
    if(validatePhoneNumber(userDetails.phno)){
        pool.query('update patient set fName = ?, lName = ?, phno = ?, gender = ?, height = ?, weight = ?, bloodType = ?, apartmentNo = ?, streetName = ?, pincode = ?',[userDetails.fName, userDetails.lName, userDetails.phno, userDetails.gender, userDetails.height, userDetails.weight, userDetails.bloodType, userDetails.apartmentNo, userDetails.streetName, userDetails.pincode], function(err){
            if(err){
                console.log(err);
                details.status = 'failed';
                res.status(404).json(details);
